                ...
                catch (Exception e)
                {
                    if (conn.IsClosed || closing)
                    {
                        if (LOG.IsDebugEnabled)
                        {
                            ...
                            LOG.Debug("An exception was thrown while closing send thread for session 0x{0:X} : {1}", conn.SessionId, e.Message);
                        }
                        break;
                    }
                    else
            ...
            do
            {
                ...
                catch (Exception ex)
                {
                    if (ex is SocketException || ex is TimeoutException)
                    {
                        ...
                        LOG.Warn(string.Format("Failed to connect to {0}:{1}.",
